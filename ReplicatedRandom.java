/*
Taken from: https://github.com/fta2012/ReplicatedRandom.git
 */
...
public class ReplicatedRandom extends Random {
    ...
    public boolean replicateState(int nextN, int n, int nextM, int m) {
        ...
        for (
                long oldSeed = oldSeedUpperN;
                // next = (old * multiplier + addend) & mask;
                // old <= (old | (multiplier - 1))
                oldSeed <= (oldSeedUpperN | ((1L << (48 - n)) - 1));
                oldSeed++
        ) {
        ...
        if (possibleSeeds.size() == 1) {
            ...
            initSeed = possibleSeeds.getFirst();
            setSeed(possibleSeeds.getFirst() ^ multiplier);
